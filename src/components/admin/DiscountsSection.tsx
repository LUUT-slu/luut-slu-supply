...
import { Tag, Plus, Pencil, Trash2, Save, Loader2, RefreshCw } from "lucide-react";
      ...
      if (isNew) {
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-discounts?action=create`,
          {
            ...
            headers: baseHeaders,
            body: JSON.stringify({
              title: editForm.title, code: editForm.code,
              value_type: editForm.value_type, value: editForm.value,
        ...
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-discounts?action=update`,
          {
            ...
            headers: baseHeaders,
            body: JSON.stringify({
              price_rule_id: editId, title: editForm.title,
              value_type: editForm.value_type, value: editForm.value,
              once_per_customer: editForm.once_per_customer,
              usage_limit: editForm.usage_limit ? parseInt(editForm.usage_limit) : null,
              starts_at: editForm.starts_at ? new Date(editForm.starts_at).toISOString() : null,
    ...
    <>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-sm">Discounts Manager</p>
            <p className="text-xs text-muted-foreground">Synced with Shopify price rules</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={fetchDiscounts} disabled={loading} className="h-8 w-8 p-0">
        ...
        {!loading && discounts.length === 0 && (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-8 text-center">
              <Tag className="mb-2 h-6 w-6 text-muted-foreground" />
              <p className="text-xs text-muted-foreground">No discounts found. Create one to get started.</p>
            </CardContent>
          </Card>
        )}
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => { if (!open) setDeleteId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete the discount from Shopify.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
